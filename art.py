...
def _theme_window(s, theme, x, y, w, h):
    if theme == "cafe":
        px(s, "wood", x + 1, y + h - 10, w - 2, 4)
        for cxp in (x + 4, x + w - 11):
            px(s, "white", cxp, y + h - 18, 7, 8)
            px(s, "skin2", cxp + 1, y + h - 17, 5, 3)
            px(s, "white", cxp + 7, y + h - 16, 2, 3)
        px(s, _shade("glass", 30), x + 7, y + h - 24, 1, 4)
    elif theme == "florist":
        px(s, "green_dk", x + 2, y + h - 12, w - 4, 6)
        cols = ["red", "yellow", "purple", "white"]
        for i, fx in enumerate(range(x + 3, x + w - 3, 5)):
            px(s, "green", fx + 1, y + h - 18, 1, 8)
            px(s, cols[i % 4], fx - 1, y + h - 22, 4, 4)
    elif theme == "books":
        cols = ["red", "shirt_blue", "green", "orange", "purple", "yellow"]
        for sy in (y + 5, y + h - 21):
            for i, bx in enumerate(range(x + 2, x + w - 3, 4)):
                px(s, cols[i % len(cols)], bx, sy, 3, 14)
                px(s, "white", bx, sy, 3, 1)
            px(s, "wood", x + 1, sy + 14, w - 2, 2)
    elif theme == "electro":
        for tx in (x + 3, x + w - 13):
            px(s, "black", tx, y + 6, 10, 8)
            px(s, "glass", tx + 1, y + 7, 8, 6)
            px(s, _shade("glass", 30), tx + 2, y + 8, 3, 2)
        px(s, "metal_dk", x + 2, y + h - 12, w - 4, 6)
    else:  # grocery
        cols = ["red", "yellow", "green", "orange", "purple", "white"]
        for sy in (y + 6, y + h - 22):
            for i, gx in enumerate(range(x + 2, x + w - 3, 4)):
                px(s, cols[i % len(cols)], gx, sy + 4, 3, 10)
                px(s, "white", gx, sy + 4, 3, 1)
            px(s, "wood", x + 1, sy + 16, w - 2, 2)


def street_building(w_tiles, h_tiles, awning="red", theme="cafe"):
    """A tall, detailed storefront drawn at 16px-per-tile (scaled up by view)."""
    W, H = w_tiles * 16, h_tiles * 16
    s = pygame.Surface((W, H), pygame.SRCALPHA)
    px(s, "wall", 0, 0, W, H)
    for y in range(0, H, 4):                      # brick courses
        px(s, "wall_dk", 0, y, W, 1)
        off = 0 if (y // 4) % 2 == 0 else 4
        for x in range(off, W, 8):
            px(s, "wall_dk", x, y, 1, 4)
    px(s, "wall_lt", 0, 0, 2, H)
    px(s, _shade("wall", -18), W - 2, 0, 2, H)
    # roof cornice + colour band
    px(s, "wall_dk", 0, 0, W, 7)
    px(s, awning, 0, 7, W, 4)
    px(s, "white", 0, 7, W, 1)
    px(s, _shade(awning, -30), 0, 11, W, 1)
    # sign plaque (the name text is drawn over this in screen space)
    px(s, "wood_dk", 6, 16, W - 12, 15)
    px(s, "cream", 7, 17, W - 14, 13)
    px(s, "wood_lt", 7, 17, W - 14, 1)
    # two big display windows with themed content
    wy, wh, half = 40, 44, W // 2
    for wx in (8, half + 2):
        ww = half - 12
        px(s, "metal_dk", wx - 2, wy - 2, ww + 4, wh + 4)
        px(s, "glass", wx, wy, ww, wh)
        _theme_window(s, theme, wx, wy, ww, wh)
        px(s, "white", wx + 2, wy + 2, 1, wh - 8)
        px(s, _shade("glass", 26), wx + 5, wy + 2, 1, wh - 10)
    # awning over the entrance
    ay = H - 44
    px(s, awning, 4, ay, W - 8, 8)
    px(s, "white", 4, ay, W - 8, 1)
    for x in range(6, W - 7, 6):
        px(s, "white", x, ay + 2, 3, 6)
    px(s, _shade(awning, -30), 4, ay + 7, W - 8, 1)
    # central door
    dw, dy = 22, H - 32
    dx = W // 2 - dw // 2
    px(s, "wood_dk", dx, dy, dw, H - dy)
    px(s, "wood", dx + 2, dy + 2, dw - 4, H - dy - 2)
    px(s, "wood_lt", dx + 2, dy + 2, dw - 4, 1)
    px(s, "glass", dx + 4, dy + 5, dw - 8, 12)
    px(s, "white", dx + 5, dy + 6, 1, 9)
    px(s, "metal_lt", dx + dw - 6, dy + 20, 2, 6)
    return s
